...
enum paramCheck {MISS, PASS};
enum match {MATCH, MISSMATCH};
...
int checkParams(param *entry, char *id, int locate);
int paramSize(symEntry *p, char *id);

int checkValAgainstParams(char *id, char *scope);
int checkTypeMismatchIds(char *lhId, char *rhT);
int checkTypeMismatchCharConsts(char *lhId, int char_type);
int checkTypeMismatchIntConsts(char *lhId, int int_type);
int checkTypeMismatchVoidConsts(char *lhId, int void_type);
